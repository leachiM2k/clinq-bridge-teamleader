...
import { createContact, deleteContact, getContacts, getOAuth2RedirectUrl, getTokens, updateContact } from "./util/teamleader";
...
class TeamleaderAdapter implements Adapter {
  ...
  public async getContacts(config: Config): Promise<Contact[]> {
    const { apiKey } = TeamleaderAdapter.validateAndReturnRequiredConfigKeys(config);
    try {
